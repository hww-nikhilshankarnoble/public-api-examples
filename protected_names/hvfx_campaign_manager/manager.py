...
# Internal
from hvfx_campaign_manager import _git_utils
...
def start_campaign(client: str, jobid: str):
    ...
    """
    config = _create_campaign_config(client, jobid)
    repo = _git_utils.clone_repo(config["url"], config["directory"])
    if not repo:
        return
    _git_utils.checkout_branch(config, repo)
    _create_project_spec(config)
    _create_campaign_directory(config)
...
def complete_campaign():
    ...
    """
    spec = _get_project_spec()
    if not spec:
        return
    config = _create_campaign_config(spec["client"], spec["jobid"])
    if not config:
        return
    repo = _git_utils.verify_local_repo(config["direcory"])
    if not repo:
        return
    branch = _verify_project_branch(config, repo)
    if not branch:
        return
...
# Private
def _get_project_spec() -> dict[str, str] | None:
    """
...
# Private
def _create_campaign_config(client: str, jobid: str) -> dict[str, str]:
    """
...
# Private
def _verify_project_branch(config: dict[str, str], repo: git.Repo) -> git.Head:
    """
...
# Private
def _create_project_spec(config: dict[str, str]) -> None:
    """
...
# Private
def _create_campaign_directory(config: dict[str, str]) -> None:
    """
