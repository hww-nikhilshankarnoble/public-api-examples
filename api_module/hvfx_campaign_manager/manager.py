...
def complete_campaign():
    ...
    if not config:
        return
    repo = git_utils.verify_local_repo(config["directory"])
    if not repo:
        return
    ...
    pr = "https://github.com/Anti-Pesto/di-campaign-{}/compare/{}?expand=1".format(
...
def get_project_spec() -> dict[str, str] | None:
    ...
    if not os.path.exists("projectspec.json"):
        print("Unable to find project spec, navigate to the root directory")
        return
